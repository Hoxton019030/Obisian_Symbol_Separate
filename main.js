...
class CJKSpacingPlugin extends obsidian.Plugin {
    async onload() {
...
function addSpaceBetweenCJKAndEn(text) {
    return text
        // 中英文之間加空格
        ...
        const numbers = p1.split(",").map((s) => s.trim()).join(" , ");
        return `{ ${numbers} }`;
    })
        // 清除重複空格
        .replace(/ +/g, " ")
...
module.exports = CJKSpacingPlugin;
